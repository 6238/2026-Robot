...
public class BetterStatusSignalCollection {
    ...
    /**
     * Creates a new collection of status signals, adding the
     * provided list of signals to the collection.
     *
     * @param signals Signals to add
     */
     ...
     * the individual status codes using {@link BaseStatusSignal#getStatus()} when this happens.
     *
    ...
    public final String getBadSignalsString() {
        ...
        for (var signal : _signals) {
            if (signal.getStatus() != StatusCode.OK) {
                ...
                badSignals.append(signal.getName())
                          .append(" (")
